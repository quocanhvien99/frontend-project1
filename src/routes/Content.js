...
const Content = () => {
    ...
    const getContent = () => {
        fetch(`${API_URL}/api/content?page=${currPage}&limit=${limitPerPage}&keyword=${keyword}`, {
            credentials: 'include'
        })
        .then(res => res.json())
        .then(data => {
        ...
        <div className="Content">
            {!isCreateNew && !isEdit.active ? (
                <div className="list-container">
                    ...
                    <div className="create-btn" onClick={() => setIsCreateNew(true)}>+ Tạo Mới</div>
                    <div className="List">
                                ...
                                <tbody>
                                    {content.data.map((item) => (
                                        <tr key={item._id}>
                                            ...
                                            <td>
                                                <span className="material-icons-outlined icon" onClick={(e) => editContent(e, item)}>
                                                    edit
                                                </span>
                                                <span className="material-icons icon" onClick={(e) => deleteContent(e, item._id)}>
                                                    delete_outline
                                                </span>
                                            </td>                                
                                        </tr>
                    ...
                    <ReactPaginate pageCount={content.countPages} pageRangeDisplayed={3} marginPagesDisplayed={2} onPageChange={onPageChangeHandle} containerClassName="pagination"/>
                </div>
            ) : <Editor keyword={keyword} setIsCreateNew={setIsCreateNew} isEdit={isEdit} setIsEdit={setIsEdit} />}
        </div>
    )
};
